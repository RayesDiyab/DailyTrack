...
def update_no_of_open(index):
    with open("Activity.json", "r") as f:
        data = json.load(f)
        data["activities"][index]["no_of_open"] = data["activities"][index]["no_of_open"] + 1
...
def json_file_manager(Activity_Name, date, end_time, hours, minutes, seconds, start_time):
    ...
    data = {"name": Activity_Name, "no_of_open":1, "time": [{"date": date, "hours": hours,"minutes": minutes, "seconds": seconds, "start_time": start_time, "end_time": end_time}]}
    time = {"date": date, "hours": hours, "minutes": minutes,"seconds": seconds, "start_time": start_time, "end_time": end_time}
    Activities = Activities_in_json()
    if Activities == []:
        data = {"activities": [{"name": Activity_Name,"no_of_open":1, "time": [{"date": date, "hours": hours,"minutes": minutes, "seconds": seconds, "start_time": start_time, "end_time": end_time}]}]}
        with open("Activity.json", "w") as f:
            json.dump(data, f, indent=3)
